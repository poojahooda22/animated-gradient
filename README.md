...
- Configure the top-level `parserOptions` property lik
```
export default tseslint.confi
  languageOptions:
    // other options..
    parserOptions: {
      project: ['./tsconfig.node.json', './tsconfig.app.jso
...
- Replace `tseslint.configs.recommended` to `tseslint.configs.recommendedTypeChecked` or `tseslint.configs.strictTypeChecke
